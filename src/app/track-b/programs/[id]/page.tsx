...
export default function GymnasticsProgramDetail({ params }: { params: { id: string } }) {
    ...
    const handleEnroll = () => {
        if (program) {
            localStorage.setItem('activeGymnasticsProgram', JSON.stringify(program));
            router.push('/track-b');
        }
    };
    ...
    if (!program) {
        return (
            <div className="container" style={{ padding: 'var(--spacing-xl)', textAlign: 'center' }}>
                ...
                <Link href="/track-b/programs" className="btn btn-primary" style={{ marginTop: '20px' }}>Back</Link>
            </div>
        );
    }
    ...
    return (
        <div className="container" style={{ padding: 'var(--spacing-lg) var(--spacing-md)' }}>
            {/* Navigation */}
            <Link href="/track-b/programs" style={{ display: 'inline-flex', alignItems: 'center', marginBottom: 'var(--spacing-md)', color: 'var(--color-text-secondary)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--spacing-xl)' }}>
                ...
                <div className="animate-fade-in" style={{ animationDelay: '0.1s' }}>
                    ...
                    <section style={{ marginBottom: 'var(--spacing-xl)' }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: '800', marginBottom: 'var(--spacing-md)', letterSpacing: '0.05em' }}>TRAINING PHASES</h2>
                ...
                <div className="animate-fade-in" style={{ animationDelay: '0.2s' }}>
                    <div style={{ background: 'var(--color-surface)', padding: 'var(--spacing-lg)', borderRadius: 'var(--radius-lg)', position: 'sticky', top: '20px', border: '1px solid var(--color-surface-hover)' }}>
                        <h3 style={{ display: 'flex', alignItems: 'center', fontSize: '1.25rem', marginBottom: 'var(--spacing-md)' }}>
                            <BookOpen size={20} style={{ marginRight: '10px' }} /> Bio-Mechanics
                        </h3>
                        <div style={{
                            lineHeight: '1.6',
                        ...
                        <h3 style={{ fontSize: '1.1rem', marginBottom: 'var(--spacing-md)' }}>Expected Results</h3>
                        <ul style={{ listStyle: 'none', padding: 0 }}>
                            <li style={{ display: 'flex', marginBottom: '12px', alignItems: 'start' }}>
                                ...
                                <span style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>Improve {program.focus} reliability.</span>
                            </li>
                            <li style={{ display: 'flex', marginBottom: '12px', alignItems: 'start' }}>
                                <CheckCircle size={18} color="var(--color-success)" style={{ marginRight: '10px', marginTop: '2px' }} />
                                <span style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>Based on Elite {program.science.includes('Cycle') ? 'SSC' : 'Leverage'} principles.</span>
                            </li>
                        </ul>
                    </div>
                </div>
